...
thread_start (void) 
{
  ...
  /* Managerial thread is created in the starting when the thread_start is called*/
...
thread_unblock (struct thread *t) 
{
  ...
  list_sort (&ready_list, cmp_priority, NULL);  //added this (Part2)
...
void thread_sleep(int64_t wakeup_at, int currentTime)
{
  ...
  list_insert_ordered(&blocked_list, &(th->elem), before, NULL);   // insert it to the sleeper list

  if(!list_empty(&blocked_list))e_next_wakeup = list_entry(list_begin(&blocked_list),struct thread,elem)->wakeup_at;
...
{
  if(!list_empty(&blocked_list)) // sleeper list is not empty
  {
    struct thread * th = thread_current(); // current running thread
    struct thread * th2 = list_entry(list_begin(&blocked_list),struct thread,elem); // thread corresponding to the head of the sleeper list
...
managerial_func (void *AUX) 
{
  ...
  while(true)
  {
    ...
    {
      struct thread * th2 = list_entry(list_begin(&blocked_list),struct thread,elem);
    ...
    if(!list_empty(&blocked_list))
      e_next_wakeup = list_entry(list_begin(&blocked_list),struct thread,elem)->wakeup_at;
